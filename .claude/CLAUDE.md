...
- **追加開封**（`withExtraPacks`）: パック数を増やすだけ。シードは変えない
  - 弾ごとに独立した乱数列（`rngFor`）を使っているので、パック数を増やしても
    その弾の続きが出るだけで、既存のカードも他の弾も一切変わらない
  - 全弾で1つの乱数列を共有すると追加開封が成立しないため、**この分離を壊さないこと**
...
### 公式デッキURL（`src/domain/deckCode.ts`）

40枚そろったら、公式サイトのデッキ詳細ページを開くリンクを出す（`OfficialDeckLink`）。

```
https://shadowverse-wb.com/ja/deck/detail/?hash=2.2.fIck.cEc-.…&lang=ja
                                                │ │ └─ カード40枚ぶん（1枚1トークン）
                                                │ └─ クラスID（1=エルフ … 7=ネメシス。カードDBと同じ）
                                                └─ 1=ローテーション / 2=アンリミテッド
```

- **フォーマットは常に 2（アンリミテッド）**。シールド戦のプールは過去弾を含むので、
  ローテーションで開くと落ちている弾のカードが弾かれる
- カードIDは **Cygames 独自の64進数4桁**。標準Base64（A〜Z 始まり）と違い
  `0-9` → `A-Z` → `a-z` → `-` → `_` の順に 0〜63 を割り当てる
- 並びは **cardId 昇順**。ただし先頭だけはサムネイルになるカードが1枚ぶん抜き出されて置かれる
  （実物のURLを解析したところ最大 cardId だったので、それに合わせている）
- 実物のURLをそのまま再現できることを `deckCode.test.ts` で検証している。
  **仕様を変えるときはこのテストが通ることを必ず確認する**
...
│   │   └── deckCode.ts   ← 公式デッキURL（独自64進数のハッシュ）を組み立てる
│   ├── data/
...
- 並び順の実装は `poolSort.compareCards` ただ1つ。カードプール・デッキ構築の一覧・
  デッキリスト・デッキ確認モーダルが全部これを使う。**新しい一覧を足すときも自前で sort しない**
  - `PoolEntry` を並べるなら `sortPoolEntries`、`LiteCard` を持っているなら `compareCards` を直接呼ぶ
- **開封やり直しとデッキ確認はアプリヘッダーに置く**（`App.tsx`）。どの画面からでも押せるようにするため。
  ヘッダーは sticky。sticky 要素の `top` は `--header-h` にそろえる
